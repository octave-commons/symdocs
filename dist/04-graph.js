...
let EXTS = new Set(String(args['--ext'])
    .split(',')
...
async function main() {
    const symDb = await openLevelCache({
        path: CACHE_PATH,
        namespace: 'symbols',
    });
    const symbolPkgs = new Set();
    for await (const [, sym] of symDb.entries())
        symbolPkgs.add(sym.pkg);
    await symDb.close();
    const pkgs = (await findWorkspacePackages(ROOT, DOMAIN_DEPTH)).filter((p) => symbolPkgs.has(p.folder.replace(/\\/g, '/')));
    if (pkgs.length === 0) {
        ...
        await Promise.all(pkgs.map(async (p) => {
            ...
            for (const f of files) {
                ...
                for (const spec of extractImportSpecifiers(src)) {
                    ...
                    if (top && internalNames.has(top) && top !== p.name)
                        specs.add(top);
                }
            }
            p.importDeps = specs;
    ...
    for (const e of edges) {
        ...
        if (!prev)
            uniq.set(k, e);
        else if (prev.kind !== 'manifest' && e.kind === 'manifest')
            uniq.set(k, e);
    }
    ...
    const nodeIds = new Map(); // npm name -> mermaid id
    pkgs.forEach((p, i) => nodeIds.set(p.name, `n${i + 1}`));
    const globalGraph = buildMermaidGlobal(pkgs, edgeList, nodeIds, (name) => path.join('./', byName.get(name).folder, 'README.md').replace(/\\/g, '/'));
    const indexLines = [
        '## Packages',
        '',
        ...pkgs
    ...
    const rdepsTable = RDEPS_TABLE
        ? renderReverseDepTable(pkgs, rdeps, (name) => `./${byName.get(name).folder}/README.md`.replace(/\\/g, '/'))
        : '';
    const domainSection = RENDER_DOMAIN_GRAPHS
        ? renderDomainGraphs(pkgs, edgeList, byName, (name, fromFolder) => path
            .relative(path.join(OUT_ROOT, fromFolder), path.join(OUT_ROOT, byName.get(name).folder, 'README.md'))
            .replace(/\\/g, '/'))
        : '';
    const globalBody = [
    ...
    for (const p of pkgs) {
        ...
        const localGraph = buildMermaidLocal(p.name, ds, rs, (name) => path
            .relative(path.dirname(pkgReadme), path.join(OUT_ROOT, byName.get(name).folder, 'README.md'))
            .replace(/\\/g, '/'));
        const body = [
            preserved.trimEnd(),
            '',
...
async function findWorkspacePackages(root, domainDepth) {
    const dirs = await listDirs(root);
    const out = [];
    async function pushIfPkg(dir) {
...
function inferDomain(folder, depth) {
...
function relativeToPkgReadmeLink(fromFolder, toFolder) {
    return path
...
function buildMermaidGlobal(pkgs, edges, idOf, hrefOf) {
...
function buildMermaidLocal(name, deps, rdeps, hrefOf) {
    const center = 'A';
...
function buildMermaidDomain(domain, pkgs, edges, hrefOf) {
    const ids = new Map();
...
/* ---------- Renderers ---------- */
function renderReverseDepTable(pkgs, rdeps, hrefOf) {
    const rows = pkgs
        .map((p) => ({
        name: p.name,
        deps: rdeps.get(p.name) ?? new Set(),
    }))
    ...
    for (const r of rows) {
        ...
        const extra = MAX_RDEPS_LIST > 0 && list.length > MAX_RDEPS_LIST
...
function renderDomainGraphs(pkgs, edges, _byName, hrefOfRelTo) {
    ...
    for (const [domain, list] of Array.from(byDomain.entries()).sort((a, b) => a[0].localeCompare(b[0]))) {
        ...
        const mermaid = buildMermaidDomain(domain, list, edges, (name) => hrefOfRelTo(name, baseFolder));
        const cross = crossDomainEdges(domain, list.map((p) => p.name), edges);
        sections.push(`### ${domain === '_root' ? 'root (packages/*)' : domain}`, '', '```mermaid', mermaid.trim(), '```', cross.length ? '**Cross-domain edges touching this domain:**' : '', cross.length ? cross.map((c) => `- ${c}`).join('\n') : '', '');
    }
    return sections.filter(Boolean).join('\n');
}
function crossDomainEdges(_domain, members, edges) {
...
export async function runGraph(overrides = {}) {
...
//# sourceMappingURL=04-graph.js.map