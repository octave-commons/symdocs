...
export async function collectSourceFiles(root, exts) {
...
export async function runScan(opts = {}) {
    ...
    const EXTS = new Set((opts.ext ?? '.ts,.tsx,.js,.jsx').split(',').map((s) => s.trim().toLowerCase()));
    ...
    for (const sf of program.getSourceFiles()) {
        ...
        if (bits[0] !== 'packages' || bits.length < 2)
            continue;
        const pkg = bits[1] ?? '';
        const moduleRel = bits.slice(2).join('/');
        function pushSymbol(kind, name, node, exported, signature) {
...
//# sourceMappingURL=01-scan.js.map